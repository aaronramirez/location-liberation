...
class bl_featured_post extends WP_Widget {
				...
				foreach( $myposts as $post ){  
			        if( has_post_thumbnail() ){ ?> 
						<div class="swiper-slide swiper-slide-large" style="width:763px;"> <?php
					        setup_postdata($post); ?>
					        	<a href="<?php echo get_permalink( $post->ID ); ?>" style="background-image: url('<?php echo get_post_image( $post->ID, 'featured_post' ); ?>')">
					        		<h3 class="post-title"><?php echo $post->post_title; 
					        		if($show_excerpt){ ?>
					        			<p class="post-excerpt"><?php echo substr(get_the_excerpt(), 0, 75); ?></p><?php
					        		} ?>
					        		</h3>
					        	</a> 
						</div><?php 
	...
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'text' => '', 'order' => '', 'tag' => 'featured', 'excerpt' => '' ) );
		...
		<p>
			<label for="<?php echo $this->get_field_id('tag'); ?>"><?php _e('Tag to get:', 'bluth'); ?></label>
